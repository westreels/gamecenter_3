  ...
  methods: {
    ...
    unbet (mode) {
      ...
      if (mode && Array.isArray(this.chips[mode]) && this.chips[mode].length) {
        this.$emit('unbet', this.chips[mode].pop())
      }
    },
    demoLose () {
      return new Promise(resolve => {
        this.dealer = true
        this.resolvers[Object.keys(this.chips)[0]] = () => {
          resolve()
          this.dealer = false
        }
        this.$nextTick(() => {
    ...
    async demoWin (wins) {
      ...
      await this.$nextTick()
      await new Promise(resolve => {
        this.resolvers[Object.keys(winsChip)[0]] = resolve
        Object.keys(this.chips).forEach(k => {
          this.$set(this.chips, k, [])
        })
      })
